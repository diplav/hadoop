...
import org.apache.hadoop.mapreduce.Reducer;
 ...
 public  class GraphReducer extends Reducer<LongWritable,Text,LongWritable,Text> {
    ...
    public void reduce(LongWritable key, Iterable<Text> values, Context context) throws IOException, InterruptedException 
    {
      ...
      //if minimal path is not found i.e. only green nodes
      ...
      //if newly minimal path is found
    		  ...
    		  for (String str1 : array){
    			  ...
    			  if(color.equals("B")) {
        			  ...
        			  final_score+=min_val;
        			  //this condition avoids the cyclic paths
        			  if(!contains(path,tarr[1])){
        				 context.write(new LongWritable(new Long(tarr[1])),new Text(tarr[1]+","+tarr[1]+","+final_score+","+path+",R"));
        				  //context.write(new LongWritable(new Long(tarr[1])),new Text(tarr[1]+","+tarr[0]+","+final_score+"+"+min_val+","+path+",R"));
        				  next=true;
        			  }
